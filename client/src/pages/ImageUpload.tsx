...
const ImageUpload = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Post to /predict-image endpoint
      ...
      if (response.ok && data.label) {
        // Save prediction result in context (use camelCase key to match Result.tsx)
        updatePredictionData('imageData', data);
        addCompletedFeature('image');
        toast.success(`Predicted Age Group: ${data.label}`);

        // Navigate to next feature or result
        const nextFeature = selectedFeatures.find(f => !completedFeatures.includes(f) && f !== 'image');
        if (nextFeature) {
